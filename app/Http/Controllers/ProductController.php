...
class ProductController extends Controller
{
    ...
    public function index()
    {
        ...
        $product = $this->productservice->getProduct($paginate);

        if ($product->isEmpty()) {
            return new ResponseResource(true, 'Product not available', null, [
                'code' => 200
            ], 200);
        }

        $productResponse = $product->map(function ($product) {
        ...
        return new ResponseResource(true, 'List of product', $productResponse, [
            'code' => 200,
    ...
    public function update(ProductRequest $request, string $uuid)
    {
        ...
        try {
            ...
            // Jika ada image yang diupload tapi terjadi error, hapus gambar baru
            if (isset($data['image'])) {
